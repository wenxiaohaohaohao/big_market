...
\max\{\pi_P,\pi_L\}\geq0.
\]
Comparative advantage is realized when the producer actually enters, produces,
and sells in equilibrium.
\end{definition}
...
\begin{proposition}[Comparative-advantage realization]
\label{prop:entry}
Suppose \(\Omega(z)>0\).  A unique minimum infrastructure level \(G_E\)
supports viable entry.  At an interior threshold:
\begin{equation}
...
\end{equation}
Hence stronger LCA lowers the infrastructure required to obtain ACA and
viable realization.  If the raw threshold is negative, the economically
truncated threshold is zero and the comparative static is weak.
\end{proposition}
...
Stronger LCA weakly lowers \(\vartheta_E,\vartheta_L,\vartheta_R\), and
\(\vartheta_Y\), with strict effects where the corresponding structural
thresholds are interior.
...
industry is more likely both to enter and to embed.  At a given production
cost, infrastructure can move the region through all three states.  The
middle region is sizable under the baseline because \(F>\bar F\).

\subsection{Development thresholds}

Figure~\ref{fig:thresholds} plots the entry, organizational, real-income, and
nominal-income thresholds against \(c\).  The curves are computed from the
...
All four thresholds weakly rise as LCA weakens.  Kinks are economically
...
baseline at \(c=0.8,z=0.5\), the raw state thresholds are:
\[
G_P^0=-0.0702,\qquad
G_L^0=0.1261,\qquad
G_X=0.2561.
...
\[
G_R=0.0463,\qquad G_Y=0.5505.
\]
The numerical ordering is strict, although the formal proposition preserves
the weaker boundary result when a discrete switch determines both thresholds.
...
nominal effect is initially negative and becomes positive only after
producer-access gains and organizational embeddedness strengthen.  The
consumption-equivalent effect crosses zero earlier because it includes the
price gain.  Higher participation shifts both effects upward, but the
consumer platform share is unchanged.  The small jumps in the income-effect
curves are the entry or \(P\)-to-\(L\) organization switches established in
the analytical model.

The appendix reports five mechanism closures.  Setting
...
services.  Finally, an equal-localization comparison shows that protection
and facilitation have different consumer-access and marginal-entry
