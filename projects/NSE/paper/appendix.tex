...
reference region.  More directly, the raw organization-specific ACA threshold
from equation~\eqref{eq:aca_index} is:
\[
G_{Ar}^0
=
...
At an interior threshold:
\[
\frac{\partial G_{Ar}^0}{\partial c}
=
\frac{1}{(\gamma+\chi z)(c+d_r)}>0.
\]
The minimum across organizations preserves strict monotonicity before
truncation at zero, by the same argument used for \(G_E\).  Holding the
reference-region and background-industry ratios fixed and applying
\(\partial c/\partial\mathcal C^P_{ij}>0\) yields the stated LCA comparative
statics for \(G_A\).

Stronger LCA therefore starts from a lower production-cost ratio and requires
weakly less \(G\) both to achieve a delivered-cost advantage and to cover the
fixed cost.  The first condition defines ACA; the second defines viability.
They need not coincide because \(\bar c_h^A\) appears only in \(G_A\), while
fixed costs appear only in \(G_E\).  Entry, production, and sales constitute
realization.
\end{proof}
...
\(G_A,G_E,G_L,G_R,G_Y\) by Propositions~\ref{prop:entry}--\ref{prop:income_thresholds};
...
\subsection{Equal-localization policy comparison}

The policy comparison holds \(z=0.5\) and \(c=0.9\) fixed.  The baseline
provides \(G=0.75\) and selects the external platform organization.  The
facilitation exercise raises shared infrastructure to \(G=0.90\).  The
protection exercise instead leaves \(G=0.75\), raises the external producer
organization cost from \(d_P=0.40\) to \(0.50\), and applies a ten-percent
consumer platform-price wedge.  Both counterfactuals select \(L\), so the
comparison holds the binary localization outcome fixed rather than comparing
different organizational regimes.

\begin{table}[htbp]
\centering
\caption{Illustrative facilitation and protection comparison}
\label{tab:policy_comparison}
\begin{tabular}{lccc}
\toprule
Outcome & Baseline & Facilitation & Protection\\
\midrule
Infrastructure \(G\) & 0.75 & 0.90 & 0.75\\
Equilibrium organization & \(P\) & \(L\) & \(L\)\\
Platform expenditure share & 0.7655 & 0.7655 & 0.6696\\
Consumer price index & 0.8287 & 0.8287 & 0.8875\\
Entry threshold \(G_E\) & 0.3300 & 0.3300 & 0.6529\\
First-round local producer income & 1.1876 & 3.7113 & 3.3919\\
Nominal local income & 3.5527 & 7.6514 & 7.3737\\
Consumption-equivalent income & 3.7942 & 8.1714 & 7.6882\\
\bottomrule
\end{tabular}
\end{table}

Facilitation and protection both induce local organization in this
...
raises local producer income.  Protection lowers the consumer platform share,
raises the price index, and nearly doubles the entry threshold relative to the
unprotected baseline.  These results are source-data outputs rather than
calibrated welfare estimates.  The exact values are stored in
\texttt{figures/source\_data/appendix\_policy\_comparison.csv}.
