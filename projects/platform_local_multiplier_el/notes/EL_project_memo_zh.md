...
Atkin et al. 研究外资零售进入对家庭福利的影响，将福利变化分为生活成本效应与名义收入效应，并分别讨论传统零售劳动收入、国内商店利润等渠道。该文为本文提供两个关键起点：

- 居民可以在不同零售渠道之间选择；
- 价格收益与本地劳动、利润损失可以同时存在。

本文不能把“低价平台替代传统商店”或“价格提高福利但本地零售受损”本身声称为新发现。本文的新增部分是：把渠道收入的空间归属嵌入地方支出固定点，使收入损失在后续支出中递归传播。
...
Moretti（2010）主要是概念和经验分析，而不是一套可以直接移植的平台模型。因此本文不是“加入 Moretti 的完整模型”，而是把本地非贸易需求反馈作为地方收入闭合条件。

### 3.4 电子商务的直接近邻：Goldmanis et al.（2010）
...
不要在查重完成前声称“首次提出地方留值率”或“首次证明平台降低地方福利”。
...
> We study a small region where integration both expands external market access and shifts household spending from local intermediaries to a lower-cost platform whose rents accrue elsewhere. Endogenous channel choice makes the local retention share enter every round of expenditure. We show that the effective local multiplier is more sensitive to retention than a first-round adjustment implies. Closed-form thresholds separate three outcomes: local nominal and real income both rise, nominal income falls while real income rises, or both fall. The marginal income drag from channel substitution is strongest during intermediate platform penetration.
...
文献矩阵至少记录：研究问题、渠道选择是否内生、是否区分本地与外部收入、是否有递归乘数、是否有福利阈值、本文与其差异。
...
如果这一命题通过首周文献查重，并且两位作者同意与后续大论文严格分工，那么该项目适合在 45 天内完成并投向 *Economics Letters*。
